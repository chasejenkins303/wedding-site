...
function useCountdown(target: Date) {
  const [diff, setDiff] = useState<number | null>(null); // null until mounted

  useEffect(() => {
    const update = () => setDiff(target.getTime() - Date.now());
    update(); // compute once immediately on mount
    const id = setInterval(update, 1000);
    return () => clearInterval(id);
  }, [target]);

  const mounted = diff !== null;
  const safeDiff = diff ?? 0;
  const done = mounted && safeDiff <= 0;
  const days = Math.max(0, Math.floor(safeDiff / 86400000));
  const hours = Math.max(0, Math.floor((safeDiff % 86400000) / 3600000));
  const mins = Math.max(0, Math.floor((safeDiff % 3600000) / 60000));
  const secs = Math.max(0, Math.floor((safeDiff % 60000) / 1000));

  return { mounted, done, days, hours, mins, secs };
}
...
export default function WeddingSite() {
  const { mounted, done, days, hours, mins, secs } = useCountdown(WEDDING_DATE);
    ...
    >
      ...
      <section>
        <div className="wrap section-padding">
          ...
          {!mounted ? (
              <div className="countdown-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)" }}>
                {["Days", "Hours", "Minutes", "Seconds"].map((label, i) => (
                  <div key={label} style={{ textAlign: "left", paddingRight: 24, borderRight: i < 3 ? `1px solid ${COLORS.line}` : "none" }}>
                    <span className="display" style={{ fontVariantNumeric: "tabular-nums", fontSize: "clamp(40px, 7vw, 84px)", fontWeight: 300, lineHeight: 1, display: "block" }}>
                      --
                    </span>
                    <span style={{ display: "block", marginTop: 14, fontSize: 13, color: COLORS.ink60 }}>{label}</span>
                  </div>
                ))}
              </div>
            ) : done ? (
              <p className="display" style={{ fontStyle: "italic", fontSize: "clamp(28px, 5vw, 44px)", color: COLORS.green }}>
                Today's the day.
              </p>
            ) : (
